...
/// Passwordless signup — one email field, "Send code" button. Same
/// Supabase call as the login screen (`signInWithOtp`) because
/// Supabase collapses signup + login into a single OTP flow. We
/// still keep two screens for the marketing copy + cross-link.
///
/// Google (and Apple on iOS) sit above the email card as one-tap
/// alternatives. Existing password-holders can still sign in through
/// the OTP flow — no branch on our side.
...
class _SignupScreenState extends ConsumerState<SignupScreen> {
  ...
  Future<void> _signUpWithGoogle() async {
    ...
    try {
      ...
      if (await _handleNetworkError(e, 'continue with Google')) {
  ...
  Future<void> _signUpWithApple() async {
    ...
    try {
      ...
      if (await _handleNetworkError(e, 'continue with Apple')) {
        if (mounted) setState(() => _loading = false);
        return _signUpWithApple();
      }
      if (mounted) {
        setState(() => _error = 'Apple sign-up failed. Please try again.');
      }
    } finally {
      if (mounted) setState(() => _loading = false);
    }
